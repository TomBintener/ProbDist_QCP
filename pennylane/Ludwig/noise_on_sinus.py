...
num_training_points = 60  # Increase the number of training points
    ...
    for iteration in range(num_iterations):
        ...
        if iteration % 10 == 0:
            ...
            for training_x, training_y in training_data:
                ...
                # Error: {error:.4f}")
            print("total: " + str(total_error) +  "average: " + str(total_error/num_training_points))
            evaluate_circuit(training_params)
    return training_params

def evaluate_circuit(final_params):
    print("Evaluating the trained circuit...")
    x_values = np.linspace(-10 * np.pi, 10 * np.pi, 100)  # Define range for plotting
    actual_ouput = f(x_values)
    predicted_outputs = [circuit(final_params, x) for x in x_values]
    plt.ylim(-2, 2)
    plt.grid(True)
    tr_x_values = [data[0] for data in training_data]
    tr_y_values = [data[1] for data in training_data]

    plt.scatter(tr_x_values, tr_y_values)
...
for i in range(1):
    noise = np.random.normal(0, 0.1, num_training_points)
    training_data = [(x, f(x)) for x in training_inputs]
    for i in range(len(training_data)):
        training_data[i] = (training_data[i][0], training_data[i][1] + noise[i])
    start_params = guess_starting_params()
    trained_params = train_circuit(start_params, training_iterations)
